...
function DayItems({ day }) {
  ...
  function generatePlanningPrompt() {
    ...
    const sections = [
      ...
      '- beer_stop, si existe, va siempre antes de lunch. Si no cabe antes, se elimina.',
      ...
      '- REGLA ESTRICTA: si incluyes beer_stop, debe ocurrir antes de lunch. Nunca coloques la cerveza después de comer ni la uses como sobremesa.',
      '- Si la comida tiene que empezar temprano por horario, reserva o encaje geográfico y ya no cabe una cerveza antes, omite beer_stop. No la traslades a después de lunch.',
      ...
      '- Si hay un bar, izakaya, terraza o local agradable que encaja naturalmente antes de comer, inclúyelo. Si no, crea antes de comer una pausa flexible para buscar algo sobre la marcha por la zona, o elimina completamente la parada de cerveza.',
      ...
      '- Distingue obligatoriamente los siguientes roles cuando aparezcan: morning_coffee, mid_morning_coffee, beer_stop, lunch, afternoon_coffee, pre_dinner_drink, dinner y after_dinner_drink.',
      '- morning_coffee debe ser la primera parada tras salir del hotel.',
      '- mid_morning_coffee es opcional y debe estar separado de beer_stop por visitas, paseo y tiempo suficiente.',
      '- afternoon_coffee debe empezar después de terminar lunch, ser la siguiente pausa real antes de otra visita principal y comenzar como máximo a las 16:00.',
      '- afternoon_coffee solo admite café o matcha real. Iyoshi Cola y cualquier refresco deben usar otro rol o quedar como optional_stop.',
      '- beer_stop, cuando exista, debe terminar antes de que empiece lunch. Si no cabe antes, se elimina.',
      ...
      '- Confirma expresamente que beer_stop aparece antes de lunch, que lunch aparece antes de afternoon_coffee, que no hay una visita principal entre lunch y afternoon_coffee y que afternoon_coffee empieza como máximo a las 16:00.',
      '- Incluye una AUDITORÍA DE SECUENCIA DE PAUSAS con el resultado: beer_stop antes de lunch; afternoon_coffee después de lunch; afternoon_coffee no más tarde de 16:00.',
      ...
      'Los roles válidos son morning_coffee, mid_morning_coffee, beer_stop, lunch, afternoon_coffee, pre_dinner_drink, dinner y after_dinner_drink.',
      'Antes de devolver el JSON valida obligatoriamente: 1) beer_stop termina antes de lunch; 2) lunch empieza normalmente entre 14:00 y 15:00; 3) afternoon_coffee empieza después de lunch; 4) no hay una visita principal entre lunch y afternoon_coffee; 5) afternoon_coffee empieza como máximo a las 16:00; 6) afternoon_coffee es café o matcha real; 7) solo hay una comida y una cena principales.',
